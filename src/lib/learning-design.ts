...
9. 不要要求教師再提供教案附件、課綱檔或評量證據檔；所需內容已在下方錨點中。`;

function buildUnitPrepCoachGemInstructions(
  project: LearningDesignProjectV1,
  designAnchorText: string,
): string {
  return `${UNIT_PREP_COACH_GEM_BEHAVIOR}

══════════════════════════════════
【本單元設計錨點｜已內嵌於 Gem 指令，無需另附檔】
單元：${project.input.unitName}
年級：${project.input.grade}｜科目：${project.input.subject}
...
export function buildUnitPrepCoachGemPackage(
  ...
  const designAnchors = {
    purpose:
      "本物件已內嵌於 Gem 自訂指令，是備課諮詢的唯一知識邊界。課堂實施建議必須引用其中的校準、學習終點、評量證據與節次藍圖，不得另起無關終點。",
    course: project.input,
    constraints: project.unitConstraints,
    labContext: buildTaiwanHighSchoolLabPrompt(project.input.subject),
    alignment: {
  ...
  const gemInstructions = buildUnitPrepCoachGemInstructions(
    project,
    designAnchorText,
  );
  const setupPrompt = `【一鍵建立「NPDL 備課諮詢 Gem」】
單元：${project.input.unitName}
建議 Gem 名稱：NPDL備課諮詢｜${project.input.unitName}

只要三步，不必上傳附件：
1. 點「一鍵複製 Gem 指令」——內容已包含行為規則＋本單元校準、學習終點、評量證據、節次藍圖。
2. 開啟 Gemini → Gem 管理 → 新增 Gem，把整段指令貼進「自訂指令／Instructions」。
3. 儲存後即可直接提問課堂實施問題。

若之後改了學習終點、評量證據或節次藍圖，請回到本工作室重新一鍵複製最新指令並覆蓋 Gem 自訂指令。
...
────────────────
【請完整貼入 Gem 自訂指令的內容｜已含設計錨點，無需另附檔】
${gemInstructions}
`,
  };
}
