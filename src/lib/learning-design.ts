...
本任務只產出教師備課教案，不需要學生學習單或教師判讀指引題本。
...
產出標準＝教師可直接列印給學生使用：版面完整、欄位齊全、表格有完整標題與表頭、作答空間足夠，貼到 Google 文件／Word 後不必再補標題或欄位。
${TAIWAN_HIGH_SCHOOL_FEASIBILITY_PROMPT}
...
A. 頁首識別區（用 Markdown 表格，表頭完整）：
   | 單元 | 第○節 | 節次標題 |
   | 班級 | 座號 | 姓名 | 日期 |
   班級／座號／姓名／日期欄位必須留白給學生手寫；不得填入任何真實學生資料。
B. 學習目標與學生版成功條件：以項目清單完整寫出，不可只寫「見教案」。
C. 本節閱讀／操作素材（若題目需要資料、數據、案例、圖表、實驗觀察欄）：必須整段內嵌於學習單內，並以有完整表頭的表格呈現；不得寫「見附件」「見投影片」或留下空白佔位。
D. 「A. 知識基礎」區（3–5 題）：
   - 區標題必須寫成「A. 知識基礎」。
...
E. 「B. NPDL 子向度思考」區（3–4 題）：
   ...
   - 最後一題必須是「官方四級進程自我判讀」，使用完整表頭表格，四級名稱與描述都要寫出，不可只寫 Level 1–4：
     | 進程等級 | 官方描述 | 我目前最接近的證據 | 勾選 |
     並另附 | 我選擇此等級的理由 | 下一步我要做的一件具體行動 |。
F. 頁尾：本節完成檢查清單（3–5 項可勾選短句）與「本節我還想問的問題」空白欄。

【每節教師判讀指引｜接在該節學生學習單之後】
- 標題必須寫「教師判讀指引｜第○節」，並註明「教師用／不發給學生」。
- 以完整表頭表格逐題呈現，欄位不得省略：
  | 題號 | 區塊（知識基礎／NPDL） | 題目目的 | 對應成功指標 | 可觀察證據 | 參考要點或回應特徵 | 未達標時的教學決策 |
- 不得把參考答案或判讀要點混入學生版。

3. 全單元對照表（文件末）：使用完整表頭
   | 節次 | 學習單題號 | 對應成功指標 | 對應證據項目 | 主要 6Cs 子向度 | 備註 |

列印與表格品質規則（強制）：
- 所有表格都必須有完整表頭列；禁止出現無標題欄、只有分隔線、或「（表格）」這類佔位文字。
- 表格欄名必須可直接理解（繁體中文），不得使用不明縮寫或僅用英文代號當唯一欄名。
- 學生學習單必須自給自足：列印後即可作答，不依賴外部連結、附件或不存在的檔案。
- 必須完整產生資料中的每一節，節次不可合併、刪除或新增。
...
- 優先黑白列印友善：避免依賴色塊才能理解的資訊；勾選框可用 □ 表示。`;
...
function buildLessonWorksheetBriefs(project: LearningDesignProjectV1) {
  ...
  return project.unitBlueprint.lessons.map((lesson) => {
    ...
    return {
      ...
      printReadyLayout: {
        goal: "教師可直接列印給學生使用；表格標題與表頭必須完整",
        studentHeaderTableColumns: ["單元", "第○節", "節次標題", "班級", "座號", "姓名", "日期"],
...
請依固定格式，直接在 Canvas 一次產生「${project.input.unitName}」共 ${project.unitBlueprint!.lessons.length} 節、可直接列印使用的全部學生學習單與教師判讀指引；每一份學習單的表格都必須有完整標題與表頭，作答欄位齊全；不要產生教案流程表。`;
  return {
    version: LESSON_PROMPT_PACKAGE_VERSION,
    projectId: project.id,
